...
| Compiler for the campaign | `tsan-line-f3deebfbab60` (branch `artifact/paper-sound` at `f3deebfbab60` = `aa8a6dd8a2e8` plus the three EA compile-time commits 412d1d513f3d, 2dcc82078a60, c38c1e7e94ec; decision of 15 Sep 15:50), gated before use: 112/112 static-count rows identical to `tsan-line-aa8a6dd8a2e8` on the 28-module corpus, zero abandoned functions (`-tsan-ea-report-abandoned`), IR suites, check-tsan x5, 12-configuration matrix and K=5 replay, `ldd` self-contained. Fallback if any gate is red: `tsan-line-aa8a6dd8a2e8` (tag `artifact-paper-sound-aa8a6dd8a2e8`), whose builds were kept. The series changes compile time only: the `MYSQLparse` extract compiles in 13.6 s instead of 1 091.6 s |
