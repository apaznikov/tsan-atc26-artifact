...
# Usage: scripts/20-static-counts.sh [build-root]     default: the binaries 40-perf.sh built under build/ (ART_BUILD)
...
root="${1:-$ART_BUILD}"
[ -d "$root" ] || { echo "no build root at $root; run scripts/40-perf.sh <app> --build-only first"; exit 2; }
failed=0
...
done < <(find "$root" -type f -perm -u+x -name '*')
[ "${failed:-0}" -eq 0 ] || { echo "$failed binaries could not be counted" >&2; exit 1; }
rows=$(( $(wc -l < "$out") - 1 ))
