...
> **Paper references.** This ledger cites the paper by section: §4.1 STC, §4.2 SWMR,
> §4.3 LO, §4.4 EA, §5 DE, §6 (peeling, DynSTC), §7 pipeline, §8 experiments, and the
> lo-proof, de-proofs, ea, peeling and concrete-semantics appendices.
>
...
with what the code does. Line numbers are those of `b4bf8b8f4613`;
fixes reference commits of the development history that the patch series in
`compiler/patches/` condenses.
...
`hygiene` — no soundness bearing. `decision:<who>` — a policy choice left to
the named party.
...
| 8 | `extern` global read here, written elsewhere (memcached `current_time`) | no linkage check | SWMR-1 — fixed def2cf34faeb (benchmark-confirmed by the authors, N=10) |
...
functions" is not applied to a *creator's* callees — `fixed:STC-1`; (b) a
function this unit never calls and never takes the address of defaults to ST,
which is only valid with whole-program visibility — `fixed:STC-2` (per-TU:
...
`__tsan_create_fiber` may release the caller's mutex — `fixed:LO-7`
(729521af8965). Cost on memcached:
...
— `fixed:LO-3/5` (c8debe12f363; reproduced as shape 13 on
297881ddc1c5). Recorded assumption: a plain `pthread_mutex_lock` does not
fail. The runtime records the lock only on success, and a program that
proceeds after a failed plain lock is outside the model, unlike the timed
variants, whose timeout is a normal path (LO-6, shape 14).
...
*Model deviation, quantified (by the authors, `tools/eviction-stress`, f80e80b1dbe6).*
...
not in the soundness claim for unbounded shadow. `decision:authors` whether
dominance elimination should be qualified this way in the paper or
...
### Preservation at the application level (measured by the authors, N=10, both stock baselines)
...
it flips with the surrounding instrumentation in both directions. The authors
...
### Bounded shadow and the optimised builds (P3/P4, measured by the authors with `evict_watch`)
...
| `getIPAFuncRetEscStatus` (488) | callee returns escaped memory? | clause 4 | pending:EA-6 (a defined callee absent from the bottom-up map answers false; the shipped code still does) | ipa-return.ll · lines 100.00%, branches 50.00% |
...
| `ThreadSanitizerPass::run` (702) | per-function driver | — | fixed:PASS-2 (58c8ace31c52: `optional<T*>` null); PASS-1 not reproduced | — |
| `ModuleThreadSanitizerPass::run` (757) | module driver; `SFI` static | — | fixed:DE-2 (2aa46b60cb86) | elim-by-dominance-inter-calls |
...
code was then committed as three commits, one per fix, and
...
sound ways to recover the yield exist and are the authors' to choose: the
...
**The summaries recover little of shell.c's prefix.** Measured on the
corpus, the summary generated from the linked shell + sqlite3 module names **nine**
...
## Performance pass: patches 0019 to 0024 and 0026 to 0028

The changes below are in the shipped patch series (`compiler/patches/`). Two measurements prompted them:
`sql_yacc.cc` compiled for 3 h 02 min under `-tsan-use-escape-analysis-global` (16–55 s without), the
escape analysis's per-function fixpoint and not its verdicts; and stock ThreadSanitizer on this tree ran
about 1.4× slower than the paper compiler's, because of the eviction counters (RT-3 above). Five more
lost-race shapes, 18 to 22, were found and fixed on the way. Each row carries its own tests and
static-count delta; the first column names the patch and the development branch it came from.

| patch (branch) | change | gate and measurement |
|---|---|---|
| 0019 (`perf/runtime-counters`) | RT-3: counters behind `COMPILER_RT_TSAN_EVICTION_STATS` (OFF), per-thread when ON | objdump per-symbol identity with upstream (above) |
| 0020 (`perf/ea-compile-time` (first commit)) | EA-10, shape 18: the out-parameter escape | `escape-callee-out-param.ll` fails on 729521af8965 at both checks |
| 0021 (`perf/ea-compile-time` (second commit)) | EA-11, shape 19: arguments of an escaped-result call | `escape-arg-of-escaped-result-call.ll` fails on 729521af8965 |
| 0026 (`perf/ea-compile-time` (third commit)) | the escape analysis's fixpoint: a block is evaluated only when a predecessor's out-state changed (returning blocks always), the object's entries are erased by range, merges insert with a hint; a fail-closed budget (`-tsan-ea-max-blocks`, `-tsan-ea-max-evals-per-block`, `-tsan-ea-max-state-entries`; abandoned = everything escaped) | verdict identity: `-tsan-ea-verify-skips` re-evaluates every skipped block and asserts it a no-op — 0 failures over both lit suites and the 28-module corpus (sqlite3.c, shell.c, memcached) in all three modes; static counts equal to the branch without the change in all nine configurations (memcached / sqlite3.c / shell.c); canonical printed states <CANON>; compile time <EATIME> |
| 0022 (`perf/ea-compile-time` (fourth commit)) | `unknown-external-summary.ll`: overlapping CHECK-DAG patterns anchored (the flake) | 3/3 |
| 0024 (`perf/stage-b` (sixth commit)) | EA-13, shape 22 — the strtol-family end pointer (found while deriving the authors' vouch lists) | `escape-libcall-out-pointer.ll` fails on 729521af8965 and d3bf9f8c39fe; static counts on the patched corpus unchanged in all four configurations (EA, SWMR, LO, sound bundle: memcached 6829 / 6928 / 6919 / 6819, sqlite3.c 55834 / 56713 / 56713 / 55834, shell.c 6303 / 6453 / 6453 / 6298) — no program in the corpus publishes an end pointer; five suites 123 passed / 1 unsupported / 0 failed |
| 0023 (`perf/ea-compile-time` (fifth commit)) | SWMR-2/LO-8, shape 20 and EA-12, shape 21 — both found on 94764fe839df | the three tests fail on 94764fe839df; static counts (opt on the `sanitize_thread`-patched corpus, EA / SWMR / LO / sound bundle): memcached and sqlite3.c unchanged in all four, shell.c −10 under EA and the bundle — accesses through never-published realloc'd buffers in `main` and `process_input` (the argument no longer escapes at the call; the result stands for the block); five suites 123/0 |
