...
| `perf/stageB-d3bf9f8c39fe/` | the earlier campaign on compiler `d3bf9f8c39fe` ("Stage B"), superseded by the one above: five applications, 14 configurations, N = 5, pinned 48 CPUs, run-major | `d3bf9f8c39fe` | see the drift note below and `docs/campaign-parameters.md`, "Earlier trees" |
...
| `preservation/` | race reports per run for SQLite, memcached and FFmpeg under stock, sound and AllOpt; L1/L2/L3 keys | several, named per tree | `*/2026-09-17-shipped-f3deebfbab60/` is the tree `CLAIMS.md` section 1, last row, cites (the memcached tree's README explains its one relocated report); `memcached-10k` excluded: a short-iteration client artefact |
...
| `tools/perf/` | `aggregate.py` and `meta_tool.py`, identical to the harness's copies, and `report.py`; `results` is a symlink to `../../perf` because the scripts locate trees relative to themselves | | |
...
set, the governor and turbo state, the load before and after, the foreign-CPU share, the peak resident set
(`max_rss_kb`) and a "disturbed" flag; `session.json` per leg records the host state.
