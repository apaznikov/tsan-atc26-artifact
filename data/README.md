...
| `perf/campaign-f3deebfbab60/best/` | FFmpeg at `-threads 16`, the thread count where the paper's transforms gain most: native, stock, DynSTC, AllOpt with peeling, AllOpt with peeling and DynSTC, and the upstream flag on stock and on that combination; N = 5, 22 Sep 2026, the campaign's set and shape | `f3deebfbab60` | the headline FFmpeg row in `CLAIMS.md` |
| `perf/campaign-f3deebfbab60/flag-<app>/` | the upstream flag `-tsan-instrument-func-entry-exit=false` on stock and on our configurations, N = 5, 22 Sep 2026, the campaign's set; measured and not claimed (`CLAIMS.md`, the section on the flag) | `f3deebfbab60` | the flag's rows are reported with the suite's gate result beside them |
| `perf/ffmpeg-threadsweep-f3deebfbab60/` | the FFmpeg thread sweep on the reference clip: 2, 4, 8 and 16 encoder threads, four configurations, plus `threads-8-dynstc/` (22 Sep 2026: native, stock, DynSTC and AllOpt with peeling and DynSTC at 8 threads), four configurations, N = 5 | `f3deebfbab60` | checked strictly too |
