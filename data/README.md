...
| `perf/campaign-f3deebfbab60/ffmpeg-t16/` | FFmpeg at `-threads 16`, the artifact's default thread count since 22 Sep 2026 (libx265's ceiling, chosen from the sweep after the campaign): native, stock, DynSTC, AllOpt with peeling, AllOpt with peeling and DynSTC, and the upstream flag on stock and on that combination; N = 5, 21-22 Sep 2026 (UTC stamps of 21 Sep), the campaign's set and shape | `f3deebfbab60` | the headline FFmpeg row in `CLAIMS.md` |
| `perf/campaign-f3deebfbab60/flag-<app>/` | the upstream flag `-tsan-instrument-func-entry-exit=false` on stock and on our configurations, for all five applications, N = 5, 21-22 Sep 2026, the campaign's set; measured and not claimed (`CLAIMS.md`, the section on the flag) | `f3deebfbab60` | the flag's rows are reported with the suite's gate result beside them |
| `perf/campaign-f3deebfbab60/sweep-<app>-<knob>/` | the concurrency curves measured after the campaign, 22-23 Sep 2026, the campaign's set and shape, N = 5: memcached's server threads at 24, 96 and 112, Redis's clients at 256 and 512, SQLite's walthread1 threads at 8, 16, 32, 48, 96 and 112, five configurations each | `f3deebfbab60` | measured and not claimed; no default changed on their evidence (`CLAIMS.md`, the curves section). `sweep-legs.log` and `sweep-legs.sh` beside them name each arm with its knob value and its times, because a cell records the thread count but not Redis's clients or SQLite's walthread1 threads |
| `perf/campaign-f3deebfbab60/wp-dynstc-<app>/` | whole-program summaries together with DynSTC, the one pairing of our own analyses the campaign never measured: Redis, SQLite and memcached, five configurations, N = 5, 22 Sep 2026 | `f3deebfbab60` | a measured null on all three; the static sites it removes are in the same section |
| `perf/sweep-apollo-f3deebfbab60/` | the same memcached and Redis curves on the second host (AMD EPYC 9115, 48 of its 64 threads pinned as 0-23,32-55), N = 5, 22-23 Sep 2026, eight arms | `f3deebfbab60` | a separate root because it is a different machine; checked strictly, one processor set per arm, with its own `legs.log` and `legs.sh` |
| `perf/ffmpeg-threadsweep-f3deebfbab60/` | the FFmpeg thread sweep on the reference clip: 2, 4, 8 and 16 encoder threads, four configurations, plus `threads-8-dynstc/` (21-22 Sep 2026: native, stock, DynSTC and AllOpt with peeling and DynSTC at 8 threads), four configurations, N = 5 | `f3deebfbab60` | checked strictly too |
...
| `perf/stageB-d3bf9f8c39fe/` | the earlier campaign, superseded by the one above and shipped as data: five applications, 14 configurations, N = 5, pinned 48 CPUs, run-major | `d3bf9f8c39fe` | measured 8-9 Sep 2026; see the drift note below |
...
| `preservation/` | race reports per run for SQLite, memcached and FFmpeg under stock, sound and AllOpt; L1/L2/L3 keys | several, named per tree | `memcached-10k` excluded: a short-iteration client artefact |
| `preservation/{sqlite,memcached}/2026-09-21-nofe-apollo-f3deebfbab60/` | the application check for the upstream flag `-tsan-instrument-func-entry-exit=false`: stock, `tsan-nofe` and AllOpt with peeling and the flag, N = 10, on the second host, with `verdict-L3.txt` (all three levels) and a README | `f3deebfbab60` | no site lost under the flag alone at any level on either application; AllOpt with peeling and the flag shows the same `conn_new` relocation as AllOpt with peeling without it (row 24) |
