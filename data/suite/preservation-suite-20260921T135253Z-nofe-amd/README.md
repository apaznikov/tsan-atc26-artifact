...
The suite gate for the upstream flag (measured, not claimed; not this paper's contribution): `scripts/30-preservation-suite.sh 5`
...
reports; that run's logs (188 MB) are not shipped. Eighteen report the race without the frames below the top one,
`suppressions_mutex` loses a suppression by function name, and `unaligned_race` reports 128 races instead of 224.
