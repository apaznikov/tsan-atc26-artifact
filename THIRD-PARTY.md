...
| ThreadSanitizer's regression suite, from the same commit | as above | Apache-2.0 with LLVM Exceptions | **vendored** in `tests/tsan`, `tests/sanitizer_common`, `tests/compiler-rt-src` and the two `lit.common.*` files, 445 files including the upstream licence text at `tests/LICENSE.TXT`, so the preservation suite runs without a compiler-rt checkout and the licence travels with the code it covers; one modification, marked in the file: `tests/lit.common.cfg.py` compares glibc versions as tuples instead of through `distutils`, which Python 3.12 no longer has (without it no `glibc-*` feature was detected, one test ran that upstream marks unsupported on this glibc and two that require glibc 2.30 were skipped) | none; our own tests are in `tests/ir` and are not third-party |
...
Paths beginning `harness/` are in the measurement harness, vendored into this repository from our lab
tree by `scripts/vendor-harness.sh` (an author-side script); `harness/MANIFEST.tsv` lists every file with
its sha256.
