...
**What our own evaluator-style runs showed.** We ran the artifact as an evaluator does nine times between
17 and 22 September 2026, five on this host and four on an AMD EPYC 9115, at N = 2 with one N = 5 re-run;
`docs/evaluator-runs.md` records each run with its table. What they establish, and what a reviewer whose own
run lands outside should read first: SQLite's headline column is wide at N = 2 in either direction (0.909 to
1.078 for AllOpt with peeling across the runs, against a shipped 1.023 [0.942, 1.061]), and the N = 5 re-run
of that row landed inside; the Redis DynSTC row sits at or just above the top of its interval in every run on
both hosts (0.961 to 0.984 here, 0.971 to 1.066 there, against 0.944 [0.927, 0.970]), always on the cost side
of 1.0 except once, which is the between-session drift this file states with every Redis row; memcached's
two-mode behaviour on the second host followed the processor-set shape, not the host, and disappeared on a
set of 24 cores with both SMT threads; and FFmpeg's rows are refused wherever the clip was regenerated, which
is the machinery working rather than a gap.
