...
| On the applications, no race site that stock ThreadSanitizer reports in every run is absent from an optimized configuration in every run | `scripts/31-preservation-apps.sh <app>` | comparative, on your own runs, never against a fixed set: detection is schedule-dependent, so the script prints the per-site frequency (k of N) under stock and under each configuration and classifies each site by the configuration's count first: KEPT if the configuration reports it in at least one run, whatever stock's frequency; LOST if the configuration never reports it and stock reports it in every run; UNDETERMINED at this N if the configuration never reports it and stock reports it only in some runs, so an unlucky schedule cannot be told from a loss without more runs; and ONLY-OPTIMIZED if the configuration reports a site stock never does, which is labelled rather than dropped because the shadow-eviction effect can produce exactly that. It exits non-zero only on LOST. **Not yet measured on the shipped compiler `f3deebfbab60`**; the run is scheduled for 17 September and this cell is replaced by its per-site tables. Every application-preservation tree shipped under `data/preservation/` is from an earlier compiler of the same lineage and is named by it (`2026-09-02-final-b4bf8b8f4613`, `2026-09-03-final-{297881ddc1c5,ad0623610ef6,def2cf34faeb}`, `2026-09-02-hardened-0de7a735`, `2026-09-02-paper-e90a3fc4`); what they showed, as observations on those compilers and not as criteria: on SQLite a union of five sites over 10 runs, the same under every configuration, none reported by stock in more than 8 of 10, so LOST is unreachable at that N on this workload; on memcached, under the four-state rule at L3 and N = 10 on `2026-09-02-final-b4bf8b8f4613` (compiler `b4bf8b8f4613`; built from the working copy rather than a frozen one, against our own rule, shown not to have mattered because all three configurations were built within 21 s of one compiler binary's timestamp), 11 sites and 11 KEPT: four sites at 10 of 10 under stock, the sound configuration and AllOpt with peeling alike, which is the one shape that certifies preservation rather than merely failing to refute it, and seven sites of the `conn_new` family at 1 of 10 under all three in lockstep; in a separate run of 30 on `def2cf34faeb`, AllOpt relocated one report within the same function once. A tree carrying no AllOpt configuration (`sqlite/2026-09-13-nofe-d3bf9f8c39fe`, a function-entry experiment on the Stage B compiler) is shipped as data and supports no row |
